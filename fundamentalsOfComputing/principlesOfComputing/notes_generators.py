...
# Generators are ways of programatically generating sequence of values in Python that can be iterated over.
# Generators are like lists, but lists exist all at once while generator elements only exist when they are iterated over.
...
def genfunc(limit):
    num = 0
    while num < limit:
        yield num # returns this value but does not end this function as occurs with return, so continues onto next iteration
        num = num + 1
