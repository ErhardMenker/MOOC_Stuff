## Combinatorics
# Combinatorics is the art of combining multi-element sets into a larger object in different ways
# Enumerations and permutations are both parts of combinatorics. Enumerations allow for repitition, while permutations do not
# Enumerations can be compressed, which means that order does not matter as long as the amount of the same elements are equivalent
# Factorials are usable in Python via the math package, by calling math.factorial(pos_intgr)
# Combinations are different ways of partitioning all possible outcomes but not counting different orders as different instances (contra permutations)
